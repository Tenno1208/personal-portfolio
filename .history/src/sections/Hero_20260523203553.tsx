...
export default function Hero() {
  ...
  // Derajat rotasi diperhalus agar pergerakan 3D terkesan elegan, tidak terlalu patah
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#03050c] px-4 sm:px-6 py-28 lg:py-0">
      
      {/* ================= BACKGROUND SYSTEM ================= */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none select-none">
        {/* Glow ambient sudut ruangan */}
        <div className="absolute top-[-50px] left-[-50px] w-[600px] h-[600px] bg-cyan-500/[0.07] rounded-full blur-[140px]" />
        <div className="absolute bottom-[-50px] right-[-50px] w-[700px] h-[700px] bg-purple-500/[0.07] rounded-full blur-[160px]" />
        
        {/* Kisi grid yang diperhalus opasitasnya agar tidak mengalihkan fokus teks */}
        <div className="absolute inset-0 bg-[linear-gradient(rgba(255,255,255,.01)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,.01)_1px,transparent_1px)] bg-[size:45px_45px]" />
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto grid lg:grid-cols-12 gap-16 lg:gap-8 items-center w-full">
        ...
        >
          ...
          {/* SUBTITLE */}
          <p className="text-zinc-500 uppercase tracking-[5px] sm:tracking-[7px] mb-4 text-[10px] sm:text-xs font-bold">
            FUTURISTIC DIGITAL ENGINEER
          </p>

          {/* HEADING UTAMA */}
          <h1 className="text-4xl sm:text-6xl md:text-7xl font-black leading-[1.08] text-white tracking-tight">
            Hi, I’m <br />
            <span className="bg-gradient-to-r from-cyan-400 via-teal-300 to-indigo-400 bg-clip-text text-transparent px-1">
              Avvatar
            </span>
          </h1>
        ...
        <div className="lg:col-span-6 flex items-center justify-center w-full relative px-2">
          ...
          <motion.div
            ...
            className="relative w-full max-w-[440px] aspect-[1/1.15] flex items-center justify-center"
          >
            {/* PORTAL NEON LINGKARAN BELAKANG KEPALA (Disesuaikan posisinya agar presisi pas di belakang potret) */}
            <div className="absolute w-[260px] sm:w-[350px] h-[260px] sm:h-[350px] top-[10%] rounded-full border border-purple-500/20 shadow-[0_0_50px_rgba(168,85,247,0.2)] bg-gradient-to-tr from-purple-500/[0.03] to-cyan-500/[0.03] animate-[pulse_3s_infinite_alternate]" />

            {/* CONTAINER UTAMA POTRET PROFILE */}
            <div className="relative w-full h-full flex items-end justify-center z-10 overflow-hidden">
              <img
                src="/profile.png"
                alt="Avvatar Profile"
                className="h-[82%] sm:h-[90%] object-contain select-none drop-shadow-[0_15px_45px_rgba(6,182,212,0.12)] filter brightness-[1.03] contrast-[1.01]"
              />
              
              {/* VIGNETTE GRADIENT (Pemberantas potongan kaku bahu bawah agar menyatu gaib dengan background) */}
              <div className="absolute bottom-0 inset-x-0 h-1/5 bg-gradient-to-t from-[#03050c] via-[#03050c]/50 to-transparent z-20 pointer-events-none" />
            </div>
            ...
            <motion.div
              ...
              className="absolute left-2 sm:-left-2 bottom-16 rounded-2xl border border-white/[0.06] bg-[#060a1c]/80 backdrop-blur-xl p-4 shadow-[0_20px_50px_rgba(0,0,0,0.4)] z-20 min-w-[125px] text-left border-l-cyan-500/40 border-l-2"
            >
              <p className="text-zinc-500 text-[10px] uppercase font-bold tracking-wider">Projects</p>
              <h3 className="text-2xl font-black text-white mt-0.5 tracking-tight">24+</h3>
            </motion.div>

            {/* WIDGET KANAN: STATUS KETERSEDIAAN KERJA */}
            <motion.div
              style={isMobile ? { y: 0 } : { translateZ: 60 }}
              className="absolute right-2 sm:-right-2 top-20 rounded-2xl border border-white/[0.06] bg-[#060a1c]/85 backdrop-blur-xl p-4 shadow-[0_20px_50px_rgba(0,0,0,0.4)] z-20 min-w-[175px] text-left border-r-purple-500/40 border-r-2"
            >
              <div className="flex items-center gap-2 mb-2.5">
                <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_10px_#34d399] animate-pulse" />
                <span className="text-emerald-400 text-[10px] font-bold uppercase tracking-wider">Available for work</span>
              </div>
              <p className="text-zinc-500 text-[9px] uppercase font-bold tracking-wide">Location</p>
              <p className="text-zinc-200 text-xs font-semibold mb-2.5">Indonesia, Earth 🌏</p>
              
              {/* MINI LOGO CONTAINER ROW */}
              <div className="flex gap-2.5 border-t border-white/[0.05] pt-2.5 text-zinc-500">
                <SiNextdotjs size={13} className="hover:text-white transition-colors duration-200" />
                <SiReact size={13} className="hover:text-[#61DAFB] transition-colors duration-200" />
                <SiTypescript size={13} className="hover:text-[#3178C6] transition-colors duration-200" />
                <SiTailwindcss size={13} className="hover:text-[#38BDF8] transition-colors duration-200" />
              </div>
            </motion.div>
